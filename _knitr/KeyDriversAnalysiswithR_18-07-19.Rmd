...
There are many variations of KDA, where similar outcomes can be achieved with say **dominance analysis** (See Budescu) or through estimating variable importance with a Random Forest model. 
...
As succinctly as possible, what a KDA does is to **infer the relative contribution or importance of several predictor variables in a regression analysis**. The keyword here is **relative** - importance is a measure of the relationship between a predictor and the outcome variable, *relative* to other predictors in the model. The regression model is based off a select number of predictor variables, and model outputs such as coefficients (or some transformed version of them) are then used to represent the importance of the input variables. 
