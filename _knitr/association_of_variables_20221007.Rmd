...
This makes the unpaired t-test less appropriate or conclusive, and we can consider other alternatives, such as the non-parametric two-samples Wilcoxon Rank-Sum test. 
...
The Wilcoxon Rank-Sum test (aka Mann Whitney U Test) provides an alternative to the unpaired two-sample t-test - for example, as we just saw, in the above where we cannot safely conclude the data to be satisfying the normality condition. The difference between the Wilcoxon Rank-Sum test and the unpaired t-test is that the former tests whether two populations have the same shape, whereas the latter parametric test compares means between two independent groups.
